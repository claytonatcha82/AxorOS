...
const pool = new Pool({
  connectionString,
  max: 2,
  application_name: 'axoros-finance-ledger-authority-lineage-reconciliation-verifier',
});
...
const commercialRecordReference = `commercial:finance-ledger-authority-lineage:verify:${suffix}`;
...
const authorities = [
  ...
  },
  {
    ...
    evidenceReferences: [`requirement:wrong:${suffix}`],
  },
  {
  ...
  },
  {
    ...
    evidenceReferences: [`finance-clearance:wrong:${suffix}`],
  },
];
...
try {
  ...
  console.log('PASS  Finance payment runtime detects broken authority-reference lineage across a complete persisted lifecycle and performs no ledger writes or mutations during reconciliation.');
} catch (error) {
