...
const pool = new Pool({
  connectionString,
  max: 2,
  application_name: 'axoros-finance-ledger-mismatch-reconciliation-verifier',
});
...
const commercialRecordReference = `commercial:finance-ledger-mismatch-reconciliation:verify:${suffix}`;
...
try {
  ...
  console.log('PASS  Finance payment runtime detects amount and currency mismatches across a complete persisted lifecycle and performs no ledger writes or mutations during reconciliation.');
} catch (error) {
