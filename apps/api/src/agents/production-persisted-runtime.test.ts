...
function createPool(initial: AgentRuntimeExecutionRecord, financeCleared: boolean, operationsReady = true): Pool {
  ...
  const query = async (sql: string, values: readonly unknown[] = []) => {
    ...
    if (sql.includes('from operations.production_readiness_decisions')) return { rowCount: operationsReady ? 1 : 0, rows: operationsReady ? [{ readiness_id: 'operations-readiness:persisted:1', commercial_record_reference: 'commercial:persisted:1', state: 'OPERATIONS_READY', contract_signed: true, onboarding_complete: true, assets_available: true, planning_complete: true, evidence_references: ['operations:persisted:1'], approved_by: 'operations_agent', approved_at: new Date('2026-08-18T17:10:00.000Z') }] : [] };
...
test('persisted Production orchestrator executes only after Finance and Operations readiness', async () => {
  const model = new CountingModelIntegration(); const runtime = createPersistedProductionRuntime({ pool: createPool(record(), true, true), integrations: integrations(model) });
...
test('persisted Production orchestrator records failure and never calls model when Finance clearance is missing', async () => {
  const model = new CountingModelIntegration(); const runtime = createPersistedProductionRuntime({ pool: createPool(record(), false, true), integrations: integrations(model) });
  const outcome = await runtime.orchestrator.execute({ executionId: 'exec-production-persisted', capabilityId: PRODUCTION_TECHNICAL_ASSISTANCE_CAPABILITY });
...
test('persisted Production orchestrator rejects Finance-only authority without Operations readiness', async () => {
  const model = new CountingModelIntegration(); const runtime = createPersistedProductionRuntime({ pool: createPool(record(false), true, true), integrations: integrations(model) });
  const outcome = await runtime.orchestrator.execute({ executionId: 'exec-production-persisted', capabilityId: PRODUCTION_TECHNICAL_ASSISTANCE_CAPABILITY });
  assert.equal(model.calls, 0); assert.equal(outcome.record.task.status, 'failed'); assert.match(outcome.record.result?.errorMessage ?? '', /operationsReadinessId/);
});
