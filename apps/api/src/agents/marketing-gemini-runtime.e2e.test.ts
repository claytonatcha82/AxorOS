...
function marketingTask(): AgentRuntimeTask {
  return {
    taskId: 'task-marketing-gemini-e2e-1',
    executionId: 'exec-marketing-gemini-e2e-1',
    originAgent: 'operations_agent',
    destinationAgent: 'marketing_agent',
    objective: 'Draft synthetic homepage marketing copy',
    priority: 'normal',
    context: { environment: 'test', dataClass: 'synthetic' },
    knowledgeReferences: ['atlas://marketing/synthetic-brief'],
    inputs: {
      brief: 'Write one concise homepage headline for Example Studio.',
      context: 'Synthetic company only. Service: web design. Tone: premium and credible.',
    },
    expectedOutput: 'Draft headline only',
    dependencies: [],
    risks: [],
    confidence: 0.95,
    ...
    attempt: 1,
    maxAttempts: 3,
    correlationId: 'corr-marketing-gemini-e2e-1',
...
class MemoryRuntimeStore implements AgentRuntimeStore {
  ...
  constructor(task: AgentRuntimeTask) {
    this.execution = {
      task,
      version: 1,
      persistedAt: '2026-08-15T10:15:00.000Z',
    };
  }
...
test('production runtime executes Marketing Agent through governed Gemini draft integration', async () => {
  let providerCalls = 0;
  ...
  const gemini: ExternalIntegration<ModelGenerationInput, ModelGenerationOutput> = {
    ...
    async execute(request) {
      ...
      return {
        ...
        status: 'drafted',
        output: {
          text: 'Web design built to make a sharper first impression.',
          model: 'gemini-3.5-flash-lite',
          finishReason: 'stop',
          inputTokens: 40,
          outputTokens: 12,
        },
        evidenceReferences: ['gemini:e2e:synthetic'],
        retryable: false,
      };
    },
  };
  ...
  const orchestrator = createAgentRuntimeOrchestrator({
    store,
    handlers,
    now: () => `2026-08-15T10:15:0${second++}.000Z`,
    createEventId: () => `marketing-gemini-event-${++eventId}`,
  });
  ...
  assert.deepEqual(outcome.record.result?.knowledgeReferences, ['atlas://marketing/synthetic-brief']);

  assert.equal(capturedInput?.prompt, 'Write one concise homepage headline for Example Studio.');
  assert.equal(capturedInput?.context, 'Synthetic company only. Service: web design. Tone: premium and credible.');
  assert.match(capturedInput?.systemInstruction ?? '', /Do not publish, send, post/);
