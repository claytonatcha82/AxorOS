...
function supportTask(): AgentRuntimeTask {
  return {
    taskId: 'task-support-gemini-e2e-1',
    executionId: 'exec-support-gemini-e2e-1',
    originAgent: 'operations_agent',
    destinationAgent: 'support_agent',
    objective: 'Analyse a synthetic support incident from verified evidence',
    priority: 'normal',
    context: { environment: 'test', dataClass: 'synthetic' },
    knowledgeReferences: ['atlas://support/synthetic-incident-policy'],
    inputs: {
      incidentBrief: 'Analyse intermittent synthetic website unavailability and recommend the next diagnostic step.',
      supportContext: 'Synthetic client only. Monitoring confirms intermittent 503 responses. No verified root cause, remediation, restoration, or production-change authorization is supplied.',
    },
    expectedOutput: 'Support incident analysis for internal review',
    dependencies: [],
    risks: [],
    confidence: 0.9,
    ...
    attempt: 1,
    maxAttempts: 3,
    correlationId: 'corr-support-gemini-e2e-1',
    createdAt: '2026-08-15T11:04:00.000Z',
...
class MemoryRuntimeStore implements AgentRuntimeStore {
  ...
  constructor(task: AgentRuntimeTask) {
    this.execution = { task, version: 1, persistedAt: '2026-08-15T11:04:00.000Z' };
  }
...
test('production runtime executes Support Agent through governed Gemini draft integration', async () => {
  let providerCalls = 0;
  ...
  const gemini: ExternalIntegration<ModelGenerationInput, ModelGenerationOutput> = {
    ...
    async execute(request) {
      ...
      return {
        ...
        status: 'drafted',
        output: {
          text: 'Verified symptom: intermittent 503 responses. Root cause remains unverified. Review current application and platform logs before proposing remediation.',
          model: 'gemini-3.5-flash-lite',
          finishReason: 'stop',
          inputTokens: 58,
          outputTokens: 27,
        },
        evidenceReferences: ['gemini:support-e2e:synthetic'],
        retryable: false,
      };
    },
  };
  ...
  const orchestrator = createAgentRuntimeOrchestrator({
    store,
    handlers,
    now: () => `2026-08-15T11:04:0${second++}.000Z`,
    createEventId: () => `support-gemini-event-${++eventId}`,
  });
  ...
  assert.equal(outcome.record.result?.agentId, 'support_agent');
  ...
  assert.deepEqual(outcome.record.result?.knowledgeReferences, ['atlas://support/synthetic-incident-policy']);

  assert.equal(capturedInput?.prompt, 'Analyse intermittent synthetic website unavailability and recommend the next diagnostic step.');
  assert.match(capturedInput?.context ?? '', /Monitoring confirms intermittent 503 responses/);
