...
function productionTask(): AgentRuntimeTask {
  return {
    taskId: 'task-production-gemini-e2e-1',
    executionId: 'exec-production-gemini-e2e-1',
    ...
    inputs: {
      ...
      technicalContext: 'Synthetic client only. Sales handoff and commercial gate are confirmed. Required pages: Home, About, Services, Projects, Contact. No deployment authorization is supplied.',
    },
    expectedOutput: 'Technical implementation draft for internal review',
    dependencies: [],
    risks: [],
    confidence: 0.96,
    ...
    attempt: 1,
    maxAttempts: 3,
    correlationId: 'corr-production-gemini-e2e-1',
    createdAt: '2026-08-15T10:56:00.000Z',
    updatedAt: '2026-08-15T10:56:00.000Z',
  };
}
...
class MemoryRuntimeStore implements AgentRuntimeStore {
  ...
  constructor(task: AgentRuntimeTask) {
    this.execution = { task, version: 1, persistedAt: '2026-08-15T10:56:00.000Z' };
  }
...
test('production runtime executes Production Agent through governed Gemini draft integration', async () => {
  ...
  const gemini: ExternalIntegration<ModelGenerationInput, ModelGenerationOutput> = {
    ...
    async execute(request) {
      ...
      return {
        ...
        status: 'drafted',
        output: {
          text: 'Plan: scaffold the approved five-page React structure, implement shared layout components, add supplied content, then run QA before any deployment step.',
          model: 'gemini-3.5-flash-lite',
          finishReason: 'stop',
          inputTokens: 61,
          outputTokens: 30,
        },
        evidenceReferences: ['gemini:production-e2e:synthetic'],
        retryable: false,
      };
    },
  };
  ...
  const orchestrator = createAgentRuntimeOrchestrator({
    store,
    handlers,
    now: () => `2026-08-15T10:56:0${second++}.000Z`,
    createEventId: () => `production-gemini-event-${++eventId}`,
  });
  ...
  assert.equal(outcome.record.result?.agentId, 'production_agent');
  ...
  assert.deepEqual(outcome.record.result?.knowledgeReferences, ['atlas://production/synthetic-requirements']);

  assert.equal(capturedInput?.prompt, 'Draft a concise implementation plan for a five-page React website.');
  assert.match(capturedInput?.context ?? '', /Sales handoff and commercial gate are confirmed/);
