...
const requirement = {
  commercialRecordReference: clearance.commercialRecordReference, gate: 'PRODUCTION_START' as const,
  requirementReference: 'requirement:synthetic:production-model-1', requirementType: 'DEPOSIT' as const,
  requiredAmountMinor: 10000, currency: 'ZAR', status: 'ACTIVE' as const,
};
const satisfaction = {
  requirementReference: requirement.requirementReference, clearanceId: clearance.clearanceId,
  commercialRecordReference: clearance.commercialRecordReference, gate: 'PRODUCTION_START' as const, satisfiedAt: clearance.verifiedAt,
};
...
function task(): AgentRuntimeTask {
  return {
    ...
    knowledgeReferences: ['atlas://production/synthetic-requirements'], inputs: { implementationBrief: 'Draft a component implementation plan for a synthetic brochure website.', technicalContext: 'Synthetic project only. Requirements are supplied for planning; no deployment or production authorization is granted.' },
    expectedOutput: 'Technical implementation draft', dependencies: [], risks: [], confidence: 0.95, approvalRequired: false, status: 'ready',
    nextAction: 'execute_destination_capability', attempt: 1, maxAttempts: 3, correlationId: 'corr-production-model-1', createdAt: '2026-08-18T08:50:00.000Z', updatedAt: '2026-08-18T08:50:00.000Z',
  };
}
