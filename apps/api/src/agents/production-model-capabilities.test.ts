...
function task(): AgentRuntimeTask {
  return {
    taskId: 'task-production-model-1',
    executionId: 'exec-production-model-1',
    ...
    inputs: {
      ...
      technicalContext: 'Synthetic project only. Requirements are supplied for planning; no deployment or production authorization is granted.',
    },
    ...
    attempt: 1,
    maxAttempts: 3,
    correlationId: 'corr-production-model-1',
...
test('Production Agent registers a governed Gemini technical-assistance capability', async () => {
  ...
  const gemini: ExternalIntegration<ModelGenerationInput, ModelGenerationOutput> = {
    ...
    async execute(request) {
      ...
      return {
        ...
        status: 'drafted',
        output: {
          text: 'Synthetic technical implementation plan.',
          model: 'gemini-3.5-flash-lite',
          finishReason: 'stop',
          inputTokens: 44,
          outputTokens: 18,
        },
        evidenceReferences: ['gemini:production-capability:synthetic'],
        retryable: false,
      };
    },
  };
  ...
  const handler = handlers.get('production_agent', PRODUCTION_TECHNICAL_ASSISTANCE_CAPABILITY);
  assert.ok(handler);

  const result = await handler.execute(task());
  assert.equal(result.status, 'completed');
  assert.equal(result.agentId, 'production_agent');
