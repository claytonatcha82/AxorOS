...
function executiveTask(): AgentRuntimeTask {
  return {
    taskId: 'task-executive-gemini-e2e-1',
    executionId: 'exec-executive-gemini-e2e-1',
    originAgent: 'operations_agent',
    destinationAgent: 'executive_agent',
    objective: 'Analyse a synthetic strategic decision without authorizing it',
    priority: 'normal',
    context: { environment: 'test', dataClass: 'synthetic' },
    knowledgeReferences: ['atlas://executive/synthetic-governance-policy'],
    inputs: {
      decisionBrief: 'Compare two synthetic pilot expansion options and recommend which should receive further Human Executive review.',
      strategicContext: 'Synthetic evidence only. Option A expands slowly with lower cost and reversibility. Option B expands faster with materially higher spend and client commitments. No spending, policy, contract, or expansion approval is supplied.',
    },
    expectedOutput: 'Strategic decision analysis for governed review',
    dependencies: [],
    risks: [],
    confidence: 0.9,
    ...
    attempt: 1,
    maxAttempts: 3,
    correlationId: 'corr-executive-gemini-e2e-1',
    createdAt: '2026-08-15T11:13:00.000Z',
    updatedAt: '2026-08-15T11:13:00.000Z',
  };
}
...
class MemoryRuntimeStore implements AgentRuntimeStore {
  ...
  constructor(task: AgentRuntimeTask) {
    this.execution = { task, version: 1, persistedAt: '2026-08-15T11:13:00.000Z' };
  }
...
test('production runtime executes Executive Agent through governed Gemini draft integration', async () => {
  let providerCalls = 0;
  ...
  const gemini: ExternalIntegration<ModelGenerationInput, ModelGenerationOutput> = {
    ...
    async execute(request) {
      ...
      return {
        ...
        status: 'drafted',
        output: {
          text: 'Option A is more reversible and lower-cost on supplied evidence. Option B introduces material spend and client commitments, so it requires Human Executive review before any authorization.',
          model: 'gemini-3.5-flash-lite',
          finishReason: 'stop',
          inputTokens: 72,
          outputTokens: 34,
        },
        evidenceReferences: ['gemini:executive-e2e:synthetic'],
        retryable: false,
      };
    },
  };
  ...
  const orchestrator = createAgentRuntimeOrchestrator({
    store,
    handlers,
    now: () => `2026-08-15T11:13:0${second++}.000Z`,
    createEventId: () => `executive-gemini-event-${++eventId}`,
  });
  ...
  assert.equal(outcome.record.result?.agentId, 'executive_agent');
  ...
  assert.deepEqual(outcome.record.result?.knowledgeReferences, ['atlas://executive/synthetic-governance-policy']);

  assert.equal(capturedInput?.prompt, 'Compare two synthetic pilot expansion options and recommend which should receive further Human Executive review.');
  assert.match(capturedInput?.context ?? '', /No spending, policy, contract, or expansion approval is supplied/);
  assert.match(capturedInput?.systemInstruction ?? '', /Do not change policy/);
  assert.match(capturedInput?.systemInstruction ?? '', /authorize spending/);
  assert.match(capturedInput?.systemInstruction ?? '', /require Human Executive review and approval/);
  assert.match(capturedInput?.systemInstruction ?? '', /advisory analysis/);
