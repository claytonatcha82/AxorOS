...
export interface PilotRuntimeOperatorOrchestrator {
  ...
  resolveApproval(input: { executionId: string; actor: 'human_executive'; decision: 'approved' | 'rejected'; reason?: string }): Promise<RuntimeExecutionOutcome>;
}
...
export interface PilotPendingApproval {
  executionId: string; destinationAgent: string; objective: string; expectedOutput: string; capabilityId: string; persistedAt: string; reason?: string;
}
...
function pendingApproval(record: AgentRuntimeExecutionRecord): PilotPendingApproval | null {
  ...
  let policy: unknown;
  if (record.task.destinationAgent === 'support_agent' && context.supportEmailApprovalPolicy) { capabilityId = SUPPORT_EMAIL_DRAFT_CAPABILITY; policy = context.supportEmailApprovalPolicy; }
  else if (record.task.destinationAgent === 'marketing_agent' && context.marketingEmailApprovalPolicy) { capabilityId = MARKETING_EMAIL_DRAFT_CAPABILITY; policy = context.marketingEmailApprovalPolicy; }
  else if (record.task.destinationAgent === 'operations_agent' && context.operationsEmailApprovalPolicy) { capabilityId = OPERATIONS_EMAIL_DRAFT_CAPABILITY; policy = context.operationsEmailApprovalPolicy; }
  if (!capabilityId || !PILOT_OPERATOR_CAPABILITIES.get(record.task.destinationAgent)?.has(capabilityId)) return null;
  const reason = policy && typeof policy === 'object' && !Array.isArray(policy) ? (policy as Record<string, unknown>).reason : undefined;
  return { executionId: record.task.executionId, destinationAgent: record.task.destinationAgent, objective: record.task.objective, expectedOutput: record.task.expectedOutput, capabilityId, persistedAt: record.persistedAt, ...(typeof reason === 'string' && reason.trim() ? { reason: reason.trim() } : {}) };
}

export function createPilotRuntimeOperatorCommand(dependencies: PilotRuntimeOperatorCommandDependencies) {
  return {
    async listPendingApprovals(limit = 25): Promise<readonly PilotPendingApproval[]> {
      if (!Number.isInteger(limit) || limit < 1 || limit > 50) throw new Error('pending approval limit must be an integer from 1 to 50.');
      if (!dependencies.store.listPendingHumanApprovals) throw new Error('pending Human Executive approval listing is not configured.');
    ...
    async listRecoveryRequired(limit = 25): Promise<readonly PilotRecoveryItem[]> {
      if (!Number.isInteger(limit) || limit < 1 || limit > 50) throw new Error('recovery queue limit must be an integer from 1 to 50.');
      if (!dependencies.store.listRecoveryRequiredExecutions) throw new Error('runtime recovery queue listing is not configured.');
    ...
    async execute(executionId: string, capabilityId: string): Promise<RuntimeExecutionOutcome> {
      ...
      if (!allowed?.has(normalizedCapabilityId)) throw new Error(`pilot operator cannot execute ${record.task.destinationAgent}:${normalizedCapabilityId}.`);
      return dependencies.orchestrator.execute({ executionId: normalizedExecutionId, capabilityId: normalizedCapabilityId });
    },

    async resolveApproval(executionId: string, decision: 'approved' | 'rejected', reason?: string): Promise<RuntimeExecutionOutcome> {
      const normalizedExecutionId = normalizedRequired(executionId, 'executionId');
      const record = await dependencies.store.getExecution(normalizedExecutionId);
      if (!record) throw new Error(`runtime execution ${normalizedExecutionId} was not found.`);
      if (!PILOT_OPERATOR_CAPABILITIES.has(record.task.destinationAgent)) throw new Error(`pilot operator cannot resolve approvals for ${record.task.destinationAgent}.`);
      if (record.task.approvalOwner !== 'human_executive') throw new Error('pilot operator can resolve only Human Executive approvals.');
      return dependencies.orchestrator.resolveApproval({ executionId: normalizedExecutionId, actor: 'human_executive', decision, ...(reason ? { reason } : {}) });
    },
  };
}
