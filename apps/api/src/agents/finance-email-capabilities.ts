...
export const FINANCE_EMAIL_DRAFT_CAPABILITY = 'create_finance_email_draft';

export interface FinanceEmailCapabilityOptions {
  integrationId?: 'email.draft' | 'email.gmail';
}

interface FinanceEmailDraftInputs {
  fromIdentity: string;
...
function readInputs(task: AgentRuntimeTask): FinanceEmailDraftInputs {
...
export function createFinanceEmailDraftHandler(integrations: IntegrationRegistry, options: FinanceEmailCapabilityOptions = {}): AgentRuntimeHandler {
  ...
  return {
    ...
    async execute(task: AgentRuntimeTask): Promise<AgentRuntimeResult> {
      if (task.destinationAgent !== 'finance_agent') throw new Error('Finance email draft capability requires destinationAgent finance_agent.');
      const input = readInputs(task);
      assertAgentMayUseEmailIdentity('finance_agent', input.fromIdentity);
      const response = await integrations.execute<EmailMessageInput, EmailDraftOutput>({
        ...
        risk: 'low',
        input,
        idempotencyKey: `finance-email-draft:${task.executionId}:${task.attempt}`,
      });
      if (response.status === 'blocked' || response.status === 'failed') throw new Error(`finance email draft integration ${response.status}.`);
      return {
        executionId: task.executionId,
        taskId: task.taskId,
        agentId: 'finance_agent',
...
export function registerFinanceEmailCapabilities(handlers: AgentRuntimeHandlerRegistry, integrations: IntegrationRegistry, options: FinanceEmailCapabilityOptions = {}): void {
  handlers.register(createFinanceEmailDraftHandler(integrations, options));
}
