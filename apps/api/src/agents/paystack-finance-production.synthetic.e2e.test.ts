...
const operationsReadiness = {
  readinessId: 'operations-readiness:stage1:synthetic:1', commercialRecordReference, state: 'OPERATIONS_READY' as const,
  contractSigned: true, onboardingComplete: true, assetsAvailable: true, planningComplete: true,
  evidenceReferences: ['operations:stage1:synthetic:ready'], approvedBy: 'operations_agent', approvedAt: paidAt,
};
...
test('signed Paystack webhook independently verifies payment, clears Finance, satisfies PRODUCTION_START, and combines with Operations readiness to authorize Production', async () => {
  ...
  const webhookStore = {
    async save(evidence: PaymentWebhookEvidence): Promise<'accepted' | 'duplicate'> {
      ...
      webhookRows.set(evidence.idempotencyKey, evidence); return 'accepted';
    },
    async get(idempotencyKey: string): Promise<PaymentWebhookEvidence | null> { return webhookRows.get(idempotencyKey) ?? null; },
  };
  ...
  const clearanceStore = {
    async save(decision: PersistedFinanceClearanceDecision): Promise<'accepted' | 'duplicate'> {
      ...
      clearanceRows.set(decision.clearanceId, decision); return 'accepted';
    },
    async get(clearanceId: string): Promise<PersistedFinanceClearanceDecision | null> { return clearanceRows.get(clearanceId) ?? null; },
  };
  ...
  const currentStateStore = {
    async apply(evidence: PaymentWebhookEvidence): Promise<'accepted'> {
      ...
      const state: PersistedFinancePaymentCurrentState = {
        provider: evidence.provider, providerPaymentReference: evidence.providerPaymentReference, commercialRecordReference: evidence.commercialRecordReference,
        paymentStatus: lifecycle.paymentStatus, authorityState: lifecycle.authorityState, reason: lifecycle.reason,
        latestEventType: evidence.eventType, latestProviderEventReference: evidence.providerEventReference,
        latestEvidenceReference: evidence.evidenceReference, latestOccurredAt: lifecycle.occurredAt,
        ...(evidence.amountMinor !== undefined ? { amountMinor: evidence.amountMinor } : {}),
        ...(evidence.currency !== undefined ? { currency: evidence.currency } : {}),
      };
      paymentStateRows.set(`${evidence.provider}:${evidence.providerPaymentReference}`, state); return 'accepted';
    },
    async get(provider: string, paymentReference: string): Promise<PersistedFinancePaymentCurrentState | null> { return paymentStateRows.get(`${provider}:${paymentReference}`) ?? null; },
  };
  ...
  const paystack = createPaystackPaymentIntegration({
    ...
    fetchImpl: async (url, init) => {
      ...
      return new Response(JSON.stringify({ status: true, message: 'Verification successful', data: { id: 9001, status: 'success', reference: providerPaymentReference, amount: amountMinor, currency, paid_at: paidAt, created_at: paidAt } }), { status: 200, headers: { 'content-type': 'application/json' } });
    },
  });

  const integrations = new IntegrationRegistry(); integrations.register(paystack);
  const clearanceWorkflow = createFinancePaymentClearanceWorkflow({ integrations, clearanceStore, paymentWebhookEvidenceStore: webhookStore });
  const eventWorkflow = createFinancePaymentEventWorkflow({ webhookStore, currentStateStore, clearanceWorkflow, paymentIntegrationId: 'payment.paystack', mode: 'sandbox' });
  const ingress = createPaystackPaymentWebhookIngress({ secretKey, currentStateStore, eventWorkflow });

  const rawBody = Buffer.from(JSON.stringify({ event: 'charge.success', data: { id: 7001, reference: providerPaymentReference, amount: amountMinor, currency, paid_at: paidAt, metadata: { axorosCommercialRecordReference: commercialRecordReference } } }));
  ...
  const clearanceId = eventResult.clearance?.decision.clearanceId; assert.ok(clearanceId);
  assert.deepEqual(eventResult.clearance?.decision.evidenceReferences, [`payment-provider:paystack:charge.success:${providerPaymentReference}`, `payment-paystack-verify:transaction:9001:${providerPaymentReference}`]);

  const requirement = { commercialRecordReference, gate: 'PRODUCTION_START' as const, requirementReference: 'requirement:stage1:synthetic:deposit', requirementType: 'DEPOSIT' as const, requiredAmountMinor: amountMinor, currency, status: 'ACTIVE' as const };
  let satisfaction: { requirementReference: string; clearanceId: string; commercialRecordReference: string; gate: 'PRODUCTION_START'; satisfiedAt: string } | null = null;
  const requirementStore = { async get(recordReference: string, gate: string) { return recordReference === commercialRecordReference && gate === 'PRODUCTION_START' ? requirement : null; } };
  const satisfactionStore = {
    async save(value: NonNullable<typeof satisfaction>): Promise<'accepted' | 'duplicate'> { if (satisfaction) return 'duplicate'; satisfaction = value; return 'accepted'; },
    async get(requirementReference: string): Promise<NonNullable<typeof satisfaction> | null> { return satisfaction?.requirementReference === requirementReference ? satisfaction : null; },
  };

  const satisfactionResult = await satisfyCommercialPaymentRequirement({ requirementStore, satisfactionStore, clearanceStore }, { commercialRecordReference, gate: 'PRODUCTION_START', clearanceId });
  ...
  const model: ExternalIntegration<ModelGenerationInput, ModelGenerationOutput> = {
    integrationId: 'model.gemini', kind: 'model', provider: 'synthetic-stage1-model', supportedModes: ['draft'], supportedOperations: ['generate_text'],
    async execute(request) { modelCalls += 1; return { integrationId: 'model.gemini', operation: request.operation, provider: 'synthetic-stage1-model', mode: request.mode, status: 'drafted', output: { text: 'Synthetic governed Production draft.', model: 'synthetic-stage1-model', finishReason: 'stop' }, evidenceReferences: ['model:synthetic-stage1:production'], retryable: false }; },
  };
