...
export function buildSalesFollowthroughTask(input: {
  ...
  const task: AgentRuntimeTask = {
    ...
    inputs: {
      salesBrief: 'Return strict JSON containing salesContext and an email subject/body. Required salesContext fields are decisionMaker, contactEmail, industry, country, businessSummary, websiteAudit, painPoints, recommendedServices, priority, confidence, previousContact, and opportunitySummary. Use this evidence policy for every field: decisionMaker and contactEmail must be explicitly sourced; industry and country may be established from explicit company/profile/address evidence; businessSummary and websiteAudit may be concise synthesis of supplied evidence when every material claim is supported; painPoints and opportunitySummary may be evidence-based Sales inferences or summaries when clearly grounded in supplied evidence and must not be presented as confirmed facts when the evidence only indicates an opportunity or likely issue; recommendedServices, priority, and confidence are derived Sales assessment outputs and must be synthesized conservatively from the supplied evidence and Atlas guidance. Do not invent facts. Do not leave a supported field absent merely because it requires synthesis. Confidence must be a number from 0 to 1 reflecting evidence strength and completeness. PreviousContact is determined only from internal operational history. If a required factual field is genuinely unsupported, omit it so downstream assessment fails closed.',
    ...
    for (const row of result.rows) {
      try {
        ...
        if (retrieval.nextAction === 'research_pending') {
          await operationalRepository.createWorkflowEvent({ eventType: 'sales_context_research_pending', actorType: 'agent', actorId: 'sales_agent', payload: { leadId, salesIntakeExecutionId: intakeExecutionId, sourceAssessmentRecordId, attemptNumber, recoveryExecutionId, missingFields: retrieval.missingFields, searchesRun: retrieval.searchesRun, searchesFailed: retrieval.searchesFailed, providerFailures: retrieval.providerFailures, outreachAuthorised: false, sendAuthorised: false, pricingAuthorised: false, commercialCommitmentAuthorised: false, nextAction: 'research_pending' } });
          console.info(JSON.stringify({ level: 'warn', event: 'sales_context_research_pending', leadId, sourceAssessmentRecordId, attemptNumber, recoveryExecutionId, searchesFailed: retrieval.searchesFailed, providerFailures: retrieval.providerFailures }));
          continue;
        }
