...
export function buildSalesFollowthroughTask(input: {
  ...
  const task: AgentRuntimeTask = {
    ...
    inputs: {
      salesBrief: 'Return strict JSON containing salesContext and an email subject/body. Required salesContext fields are decisionMaker, contactEmail, industry, country, businessSummary, websiteAudit, painPoints, recommendedServices, priority, confidence, previousContact, and opportunitySummary. Populate factual fields only when directly supported by supplied evidence. recommendedServices, priority, and confidence are derived Sales assessment outputs: synthesize them from the supplied evidence and Atlas guidance rather than treating them as web facts. Confidence must be a number from 0 to 1 reflecting the strength and completeness of the evidence. If the evidence is genuinely insufficient to make a defensible derived assessment, omit that derived field. Keep arrays concise and evidence-backed. Do not omit a supported factual field merely because it is inconvenient to extract.',
...
export function createSalesQualifiedLeadFollowthroughService(pool: Pool, integrations: IntegrationRegistry) {
  ...
  registerModelRuntimeCapability(handlers, integrations, { agentId: 'sales_agent', capabilityId: SALES_QUALIFIED_LEAD_FOLLOWTHROUGH_CAPABILITY, integrationId: modelIntegrationId, mode: 'draft', promptInputKey: 'salesBrief', contextInputKey: 'salesContext', systemInstruction: ['You are the AxorOS Sales Agent operating in governed internal draft mode.', 'Return JSON only with keys salesContext and email.', 'Use only facts explicitly present in the supplied persisted lead, qualification, intake result, additional public-web evidence, internal operational history, or Atlas references.', 'Treat internal operational history as authoritative for AxorOS contact history. Do not use public-web results to infer whether AxorOS previously contacted the lead.', 'If internal operational history contains no recorded AxorOS outreach/contact event for the lead, previousContact must be false. If it contains a recorded completed outreach/contact event, previousContact may be true only when the event explicitly supports that conclusion.', 'Never invent a decision maker, industry, country, business summary, website audit, pain point, pricing, discount, budget, contract term, delivery promise, or approval.', 'recommendedServices, priority, and confidence are Sales synthesis outputs, not public-web facts. Derive them conservatively from the supplied evidence and Atlas guidance. Confidence must be numeric from 0 to 1 and should reflect evidence strength and completeness. Do not fabricate unsupported factual claims merely to fill a field.', 'If the evidence does not support a factual sales context field, leave it absent so the downstream assessment fails closed rather than guessing.', 'When additional public-web evidence is supplied, inspect it field-by-field and extract directly supported facts before deciding a factual field is missing.', 'The email is an internal candidate draft for human review; do not send it and do not imply outreach authority.', 'Do not include prices or commercial commitments unless explicitly present in the supplied evidence.'].join(' '), maxOutputTokens: 2000, temperature: 0.2 });
