...
export interface PilotRuntimeControlPlaneDependencies {
  ...
  operatorCommand: {
    ...
    resolveApproval(executionId: string, decision: 'approved' | 'rejected', reason?: string): Promise<RuntimeExecutionOutcome>;
  };
...
function validExecuteBody(body: Record<string, unknown>): body is { executionId: string; capabilityId: string } {
  ...
  return keys.length === 2 && keys.every((key) => key === 'executionId' || key === 'capabilityId')
    && typeof body.executionId === 'string' && Boolean(body.executionId.trim())
    && typeof body.capabilityId === 'string' && Boolean(body.capabilityId.trim());
}

function validApprovalBody(body: Record<string, unknown>): body is { executionId: string; decision: 'approved' | 'rejected'; reason?: string } {
  const keys = Object.keys(body);
...
function outcomeBody(outcome: RuntimeExecutionOutcome): Record<string, unknown> {
  return { executionId: outcome.record.task.executionId, destinationAgent: outcome.record.task.destinationAgent, status: outcome.record.task.status,
    approvalRequired: outcome.record.task.approvalRequired, approvalOwner: outcome.record.task.approvalOwner ?? null,
    nextAction: outcome.record.task.nextAction, resultStatus: outcome.record.result?.status ?? null, replayed: outcome.replayed };
}

export function createPilotRuntimeControlPlaneRequestHandler(dependencies: PilotRuntimeControlPlaneDependencies): RequestListener {
  return async (request, response) => {
    if (!isPilotRuntimeControlPath(request.url)) { dependencies.fallback(request, response); return; }
    ...
    const isReadOnlyList = request.url === PILOT_RUNTIME_PENDING_APPROVALS_PATH || request.url === PILOT_RUNTIME_RECOVERY_PATH;
    if ((isReadOnlyList && request.method !== 'GET') || (!isReadOnlyList && request.method !== 'POST')) {
      sendJson(response, 405, { ok: false, error: { code: 'method_not_allowed', message: 'Method is not allowed.' } }, { allow: isReadOnlyList ? 'GET,OPTIONS' : 'POST,OPTIONS', ...corsHeaders }); return;
    }
    ...
    if (request.url === PILOT_RUNTIME_PENDING_APPROVALS_PATH) {
      try { const approvals = await dependencies.operatorCommand.listPendingApprovals(); sendJson(response, 200, { ok: true, data: { approvals } }, corsHeaders); }
      catch (error) { sendJson(response, 400, { ok: false, error: { code: 'runtime_pending_approvals_rejected', message: error instanceof Error ? error.message : 'Pending approval listing failed.' } }, corsHeaders); }
      return;
    }

    if (request.url === PILOT_RUNTIME_RECOVERY_PATH) {
      try { const recovery = await dependencies.operatorCommand.listRecoveryRequired(); sendJson(response, 200, { ok: true, data: { recovery } }, corsHeaders); }
      catch (error) { sendJson(response, 400, { ok: false, error: { code: 'runtime_recovery_listing_rejected', message: error instanceof Error ? error.message : 'Runtime recovery listing failed.' } }, corsHeaders); }
      return;
    }
    ...
    if (request.url === PILOT_RUNTIME_EXECUTE_PATH) {
      if (!validExecuteBody(body)) { sendJson(response, 400, { ok: false, error: { code: 'invalid_runtime_execute_command', message: 'Request body must contain only non-empty executionId and capabilityId.' } }, corsHeaders); return; }
      try { const outcome = await dependencies.operatorCommand.execute(body.executionId, body.capabilityId); sendJson(response, 200, { ok: true, data: outcomeBody(outcome) }, corsHeaders); }
      catch (error) { sendJson(response, 400, { ok: false, error: { code: 'runtime_execute_command_rejected', message: error instanceof Error ? error.message : 'Runtime execute command failed.' } }, corsHeaders); }
      return;
    }

    if (!validApprovalBody(body)) { sendJson(response, 400, { ok: false, error: { code: 'invalid_runtime_approval_command', message: 'Request body must contain executionId, approved/rejected decision, and optionally a non-empty reason.' } }, corsHeaders); return; }
    try { const outcome = await dependencies.operatorCommand.resolveApproval(body.executionId, body.decision, body.reason); sendJson(response, 200, { ok: true, data: outcomeBody(outcome) }, corsHeaders); }
    catch (error) { sendJson(response, 400, { ok: false, error: { code: 'runtime_approval_command_rejected', message: error instanceof Error ? error.message : 'Runtime approval command failed.' } }, corsHeaders); }
  };
}
