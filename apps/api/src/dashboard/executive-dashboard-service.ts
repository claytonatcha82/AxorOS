...
type DashboardQueryResult = QueryResult<Record<string, unknown>>;
  ...
  return {
    async snapshot(): Promise<ExecutiveDashboardSnapshot> {
      const queries: ReadonlyArray<readonly [string, string]> = [
        ...
        ['salesPipeline', `with runtime_sales as (select distinct on (task->'context'->>'leadId') task->'context'->>'leadId' as lead_id, coalesce(nullif(task->'context'->>'company', ''), nullif(task->'context'->>'companyName', ''), 'Lead ' || (task->'context'->>'leadId')) as company, nullif(task->'context'->>'contactName', '') as contact_name, nullif(task->'context'->>'contactEmail', '') as contact_email, case when nullif(task->'context'->>'qualificationScore', '') ~ '^\\d+$' then (task->'context'->>'qualificationScore')::int else null end as qualification_score, task->>'objective' as runtime_objective, task->>'status' as runtime_status, coalesce(nullif(task->>'nextAction', ''), nullif(task->'context'->>'nextAction', '')) as runtime_next_action, task->>'approvalRequired' = 'true' as runtime_human_review_required, persisted_at from runtime.agent_executions where destination_agent = 'sales_agent' and nullif(task->'context'->>'leadId', '') is not null order by task->'context'->>'leadId', persisted_at desc), latest_events as (select distinct on (payload->>'leadId') payload->>'leadId' as lead_id, coalesce(nullif(payload->>'company', ''), 'Lead ' || (payload->>'leadId')) as company, nullif(payload->>'contactName', '') as contact_name, nullif(payload->>'contactEmail', '') as contact_email, case when nullif(payload->>'existingLeadScore', '') ~ '^\\d+$' then (payload->>'existingLeadScore')::int else null end as qualification_score, payload->>'assessmentStatus' as assessment_status, case when jsonb_typeof(payload->'missingInformation') = 'array' then payload->'missingInformation' else jsonb_build_array() end as missing_information, event_type, created_at, coalesce(payload->>'nextAction', '') as next_action, event_type = 'sales_internal_outreach_draft_recorded' as draft_ready, payload->>'humanReviewRequired' = 'true' as human_review_required, payload->>'outreachAuthorised' = 'true' as outreach_authorised, payload->>'sendAuthorised' = 'true' as send_authorised, payload->>'pricingAuthorised' = 'true' as pricing_authorised, payload->>'commercialCommitmentAuthorised' = 'true' as commercial_commitment_authorised from operational.workflow_events where actor_id = 'sales_agent' and payload ? 'leadId' and event_type in ('sales_opportunity_assessment_recorded','sales_internal_outreach_draft_recorded') order by payload->>'leadId', created_at desc), combined as (select coalesce(e.lead_id, r.lead_id) as lead_id, coalesce(e.company, r.company) as company, coalesce(e.contact_name, r.contact_name) as contact_name, coalesce(e.contact_email, r.contact_email) as contact_email, coalesce(e.qualification_score, r.qualification_score) as qualification_score, e.assessment_status, coalesce(e.missing_information, jsonb_build_array()) as missing_information, e.draft_ready, coalesce(e.human_review_required, r.runtime_human_review_required) as human_review_required, coalesce(e.outreach_authorised, false) as outreach_authorised, coalesce(e.send_authorised, false) as send_authorised, coalesce(e.pricing_authorised, false) as pricing_authorised, coalesce(e.commercial_commitment_authorised, false) as commercial_commitment_authorised, coalesce(e.created_at, r.persisted_at) as last_updated, nullif(coalesce(e.next_action, r.runtime_next_action, ''), '') as next_action, r.runtime_objective, r.runtime_status from latest_events e full outer join runtime_sales r on r.lead_id = e.lead_id) select lead_id, company, contact_name, contact_email, qualification_score, case when draft_ready then 'REVIEW' when assessment_status in ('context_incomplete','context_complete') then 'ACTIVE' when runtime_status in ('queued','ready','in_progress','waiting','blocked') then 'ACTIVE' when runtime_status = 'review' then 'REVIEW' when runtime_status = 'failed' then 'FAILED' else 'IDLE' end as activity, case when draft_ready then 'Outreach draft ready for Human Executive review' when assessment_status = 'context_incomplete' then 'Sales assessment incomplete · context retrieval / reassessment required' when assessment_status = 'context_complete' then 'Sales assessment complete · outreach preparation' when runtime_status in ('queued','ready','in_progress','waiting','blocked') then 'Sales intake / internal processing' when runtime_status = 'review' then 'Sales execution awaiting review' when runtime_status = 'failed' then 'Sales execution failed · recovery required' else 'Sales intake / internal processing' end as stage, case when draft_ready then 'Prepare outreach draft for Human Executive review' when assessment_status = 'context_incomplete' then 'Retrieve missing Sales context and reassess' when assessment_status = 'context_complete' then 'Prepare governed outreach context' when runtime_objective is not null and runtime_objective <> '' then runtime_objective else 'Continue internal Sales processing' end as objective, assessment_status, missing_information, human_review_required, outreach_authorised, send_authorised, pricing_authorised, commercial_commitment_authorised, last_updated, next_action from combined order by last_updated desc`],
        ['projects', `select count(*)::int as total, count(*) filter (where status = 'active')::int as active, count(*) filter (where status = 'qa')::int as qa, count(*) filter (where status = 'awaiting_approval')::int as awaiting_approval, count(*) filter (where status = 'delivered')::int as delivered from operational.projects`],
        ['financeExpected', `select currency, coalesce(sum(required_amount_minor), 0)::bigint as amount_minor from finance.commercial_payment_requirements where status = 'ACTIVE' group by currency order by currency`],
        ['financeReceived', `select currency, coalesce(sum(amount_minor), 0)::bigint as amount_minor from finance.clearance_decisions where state = 'FINANCE_CLEARED' group by currency order by currency`],
        ['financeRecurring', `select currency, round(sum(case billing_frequency when 'MONTHLY' then amount_minor::numeric when 'QUARTERLY' then amount_minor::numeric / 3 when 'ANNUAL' then amount_minor::numeric / 12 end))::bigint as amount_minor from finance.subscriptions where status = 'ACTIVE' group by currency order by currency`],
        ['financeExpenses', `select currency, round(sum(case when billing_type = 'RECURRING' then case billing_period when 'MONTHLY' then amount_minor::numeric when 'QUARTERLY' then amount_minor::numeric / 3 when 'ANNUAL' then amount_minor::numeric / 12 end when billing_type = 'ONE_TIME' and status = 'PLANNED' and expense_date >= date_trunc('month', current_date)::date and expense_date < (date_trunc('month', current_date) + interval '1 month')::date then amount_minor::numeric else 0 end))::bigint as amount_minor from finance.expenses where status <> 'CANCELLED' group by currency having sum(case when billing_type = 'RECURRING' then amount_minor::numeric when billing_type = 'ONE_TIME' and status = 'PLANNED' and expense_date >= date_trunc('month', current_date)::date and expense_date < (date_trunc('month', current_date) + interval '1 month')::date then amount_minor::numeric else 0 end) > 0 order by currency`],
        ['financeRequirements', `select count(*)::int as pending from finance.commercial_payment_requirements where status = 'ACTIVE'`],
        ['financeClearances', `select count(*)::int as cleared from finance.clearance_decisions where state = 'FINANCE_CLEARED'`],
        ['approvals', `select count(*)::int as pending from runtime.agent_executions where status = 'review' and task->>'approvalRequired' = 'true' and task->>'approvalOwner' = 'human_executive'`],
