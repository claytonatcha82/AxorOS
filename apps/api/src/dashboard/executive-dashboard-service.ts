  ...
  return {
    async snapshot(): Promise<ExecutiveDashboardSnapshot> {
      const [clientResult, leadResult, salesResult, projectResult, financeExpectedResult, financeReceivedResult,
        ...
        pool.query(`select destination_agent, count(*)::int as total_executions, count(*) filter (where status in ('queued','ready','in_progress','waiting','blocked'))::int as active_executions, count(*) filter (where status = 'completed')::int as completed_executions, count(*) filter (where status = 'review')::int as review_executions, count(*) filter (where status = 'failed')::int as failed_executions, max(persisted_at) as latest_activity_at, (array_agg(task->>'objective' order by persisted_at desc))[1] as latest_objective from runtime.agent_executions group by destination_agent`, []),
