...
function validSendGateOnlyBody(body: Record<string, unknown>): body is { sendGateRecordId: string } {
  const keys = Object.keys(body);
  return keys.length === 1 && keys[0] === 'sendGateRecordId' && typeof body.sendGateRecordId === 'string' && Boolean(body.sendGateRecordId.trim());
}
function validDraftReviewBody(body: Record<string, unknown>): body is { draftRecordId: string; decision: SalesOutreachDraftReviewDecision } {
  const keys = Object.keys(body);
  return keys.length === 2 && keys.includes('draftRecordId') && keys.includes('decision') && typeof body.draftRecordId === 'string' && Boolean(body.draftRecordId.trim()) && (body.decision === 'approved' || body.decision === 'rejected');
}
function validSupervisedSendGateBody(body: Record<string, unknown>): body is { draftReviewRecordId: string; decision: SalesSupervisedSendDecision } {
  const keys = Object.keys(body);
...
function validSalesContext(value: unknown): value is SalesOpportunityContext {
  ...
  if (!validOptionalText(context.decisionMaker) || !validOptionalText(context.industry) || !validOptionalText(context.country) || !validOptionalText(context.businessSummary) || !validOptionalText(context.websiteAudit) || !validOptionalTextList(context.painPoints) || !validOptionalTextList(context.recommendedServices) || !validOptionalText(context.priority) || !validOptionalText(context.previousContact)) return false;
  if (context.confidence !== undefined && (typeof context.confidence !== 'number' || !Number.isFinite(context.confidence) || context.confidence < 0 || context.confidence > 1)) return false;
  return true;
}
function validAssessmentBody(body: Record<string, unknown>): body is { executionId: string; salesContext?: SalesOpportunityContext } {
  const keys = Object.keys(body);
  if (!keys.every((key) => key === 'executionId' || key === 'salesContext') || !keys.includes('executionId')) return false;
...
export function createSalesIntakeControlPlaneRequestHandler(dependencies: SalesIntakeControlPlaneDependencies): RequestListener {
  return async (request, response) => {
    if (!isSalesControlPath(request.url)) { dependencies.fallback(request, response); return; }
