...
function buildDocumentInput(sourcePath: string, sourceVersion: string, lastModified: string, markdown: string): { document: KnowledgeDocumentInput; chunks: ReturnType<typeof chunkAtlasDocument> } {
  ...
  const statusRaw = stringValue(metadata.status, 'active') as KnowledgeStatus;
  const authorityRaw = stringValue(metadata.authority_level, 'reference') as AuthorityLevel;
  if (!statuses.has(statusRaw)) throw new Error(`Invalid knowledge status: ${statusRaw}`);
  if (!authorities.has(authorityRaw)) throw new Error(`Invalid authority_level: ${authorityRaw}`);
  ...
  const document: KnowledgeDocumentInput = {
    ...
    documentType: stringValue(metadata.document_type, 'reference'),
    knowledgeDomain: stringValue(metadata.knowledge_domain, 'general'),
    status: statusRaw,
    priority: Math.max(0, Math.min(100, Math.round(numberValue(metadata.priority, 50)))),
    authorityLevel: authorityRaw,
    allowedAgents: stringArray(metadata.allowed_agents),
    applicableTasks: stringArray(metadata.applicable_tasks),
    serviceTypes: stringArray(metadata.service_types),
    technology: stringArray(metadata.technology),
    projectStage: stringArray(metadata.project_stage),
    securityClassification: stringValue(metadata.security_classification, 'internal') as KnowledgeDocumentInput['securityClassification'],
  ...
  if (!['public','internal','restricted','confidential'].includes(document.securityClassification)) {
    throw new Error(`Invalid security_classification: ${document.securityClassification}`);
  }
