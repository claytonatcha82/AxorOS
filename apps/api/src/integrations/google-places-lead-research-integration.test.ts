...
test('Google Places lead research rejects results with missing type data', async () => {
  const integration = createGooglePlacesLeadResearchIntegration({
    apiKey: 'test-google-places-key',
    fetchImpl: async () => new Response(JSON.stringify({ places: [{ id: 'place-unknown', displayName: { text: 'Unclassified Result' } }] }), { status: 200 }),
  });
...
test('Google Places lead research validates the configured region code', () => {
  assert.throws(() => createGooglePlacesLeadResearchIntegration({ apiKey: 'test-key', regionCode: 'South Africa' }), /regionCode must be a two-letter ISO 3166-1 alpha-2 code/);
});

test('Google Places lead research accepts an explicit region code', async () => {
  let capturedInit: RequestInit | undefined;
  const integration = createGooglePlacesLeadResearchIntegration({ apiKey: 'test-key', regionCode: 'NL', fetchImpl: async (_url, init) => { capturedInit = init; return new Response(JSON.stringify({ places: [] }), { status: 200 }); } });
...
test('Google Places lead research passes pagination token and returns next page token', async () => {
  ...
  const integration = createGooglePlacesLeadResearchIntegration({
    ...
    fetchImpl: async (_url, init) => {
      capturedInit = init;
      return new Response(JSON.stringify({ places: [{ id: 'place-456', displayName: { text: 'Second Page Plumbing' }, formattedAddress: 'Durban, South Africa', types: ['plumber'] }], nextPageToken: 'next-page-token-123' }), { status: 200 });
    },
  });
  const result = await integration.execute(request({ input: { query: 'plumbers in Durban South Africa', maxResults: 20, pageToken: 'current-page-token-123' } }));
  assert.equal(result.status, 'succeeded');
  assert.equal(result.output.nextPageToken, 'next-page-token-123');
  assert.deepEqual(JSON.parse(String(capturedInit?.body)), { textQuery: 'plumbers in Durban South Africa', maxResultCount: 20, regionCode: 'ZA', pageToken: 'current-page-token-123' });
});

test('Google Places lead research accepts long pagination tokens from the provider', async () => {
  let calls = 0;
  const integration = createGooglePlacesLeadResearchIntegration({ apiKey: 'test-key', fetchImpl: async () => { calls += 1; return new Response(JSON.stringify({ places: [] }), { status: 200 }); } });
  const result = await integration.execute(request({ input: { query: 'businesses in Durban', pageToken: 'x'.repeat(201) } }));
  assert.equal(result.status, 'succeeded');
...
test('Google Places lead research blocks excessively large pagination tokens', async () => {
  let calls = 0;
  const integration = createGooglePlacesLeadResearchIntegration({ apiKey: 'test-key', fetchImpl: async () => { calls += 1; return new Response('{}', { status: 200 }); } });
  const result = await integration.execute(request({ input: { query: 'businesses in Durban', pageToken: 'x'.repeat(4097) } }));
...
test('Google Places lead research marks network failures retryable without evidence', async () => {
  const integration = createGooglePlacesLeadResearchIntegration({ apiKey: 'test-key', fetchImpl: async () => { throw new Error('network down'); } });
  const result = await integration.execute(request());
