...
export class SalesEmailSendAttemptPostgresStore {
  ...
  async getBySendGateRecordId(sendGateRecordId: string): Promise<SalesEmailSendAttempt | null> {
    const result = await this.pool.query<PersistedSalesEmailSendAttempt>(
      `select send_gate_record_id, idempotency_key, status, provider_message_id, error_message, created_at, updated_at
  ...
  async reserve(sendGateRecordId: string, idempotencyKey: string): Promise<SalesEmailSendAttempt> {
    const result = await this.pool.query<PersistedSalesEmailSendAttempt>(
      `insert into operational.sales_email_send_attempts (send_gate_record_id, idempotency_key, status)
       values ($1, $2, 'reserved')
       on conflict do nothing
       returning send_gate_record_id, idempotency_key, status, provider_message_id, error_message, created_at, updated_at`,
      [sendGateRecordId, idempotencyKey],
    );
  ...
  async markFailed(sendGateRecordId: string, errorMessage: string): Promise<SalesEmailSendAttempt> {
    const result = await this.pool.query<PersistedSalesEmailSendAttempt>(
      `update operational.sales_email_send_attempts
          set status = 'failed', error_message = $2, provider_message_id = null, updated_at = now()
        where send_gate_record_id = $1 and status = 'reserved'
        returning send_gate_record_id, idempotency_key, status, provider_message_id, error_message, created_at, updated_at`,
